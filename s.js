...
/**
 * ===================================================================
 * 商品查詢小工具 vers.sonnent0703 - 優化版
 * * 優化重點：
 * - 修正記憶體洩漏問題：正確清理事件監聽器和計時器
 * - 統一錯誤處理機制：新增 ErrorBoundaryModule
 * - 效能優化：減少 DOM 操作，優化批量更新
 * - 改善資源管理：Toast 和 Modal 生命周期管理
 * - 保持所有欄位名稱、API接口、功能邏輯完全不變
 * ===================================================================
 */

// 清理舊工具實例
(() => {
  ['planCodeQueryToolInstance', 'planCodeToolStyle', 'pctModalMask', 'pct-toast-container', 'pct-cancel-query-btn']
    .forEach(id => document.getElementById(id)?.remove());
  document.querySelectorAll('.pct-modal-mask, .pct-toast').forEach(el => el.remove());
})();
...
const UtilsModule = (() => {
  ...
  const errorHandler = {
    handle(error, context = '', showToast = true) {
      ...
      UIModule.Toast.show(message, 'error');
    },
...
const StateModule = (() => {
  ...
  const cleanup = () => {
    ...
    if (state.currentQueryController) {
      ...
      state.currentQueryController = null;
    }
...
const ApiModule = (() => {
  ...
  const queryMultiplePlanCodes = async (planCodes, signal) => {
    const BATCH_SIZE = config.UI_SETTINGS.BATCH_SIZE;
    const DELAY_BETWEEN_BATCHES = config.UI_SETTINGS.BATCH_DELAY;
    const allRecords = [];
    ...
    for (let i = 0; i < planCodes.length; i += BATCH_SIZE) {
      ...
      const batchPromises = batch.map(async planCode => {
        return utils.errorHandler.withErrorHandling(async () => {
          const params = {
            planCode,
            currentPage: 1,
      ...
      allRecords.push(...successfulResults);
      
      // 批次間延遲，避免過度請求
      if (i + BATCH_SIZE < planCodes.length) {
...
const UIModule = (() => {
  ...
  const ToastManager = {
    ...
    show(message, type = 'info', duration = config.UI_SETTINGS.TOAST_DURATION, persistent = false) {
      ...
      if (persistent && state.persistentToastId) {
        ...
        this.activeToasts.delete(state.persistentToastId);
      }
      
      const toastId = `pct-toast-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      ...
      if (persistent) {
        stateModule.set({ persistentToastId: toastId });
      } else {
        setTimeout(() => {
          this.remove(toastId);
        }, duration);
    ...
    remove(toastId) {
      ...
      if (toast?.classList.contains('pct-toast-show')) {
    ...
    hide() {
      ...
      if (state.persistentToastId) {
        this.remove(state.persistentToastId);
  ...
  const ModalManager = {
    ...
    close() {
      [config.TOOL_ID, 'pctModalMask'].forEach(id => {
      ...
      this.dragCleanupFunctions.clear();
    ...
    enableDragging(modal) {
      ...
      // 儲存清理函數
      this.dragCleanupFunctions.add(cleanup);
    ...
    const loadDetailsInBackground = async (processedData, forceFetch, onProgress) => {
      ...
      for (let i = 0; i < processedData.length; i += BATCH_SIZE) {
        ...
        if (i + BATCH_SIZE < processedData.length) {
          await new Promise(resolve => setTimeout(resolve, config.UI_SETTINGS.BATCH_DELAY));
        }
      }
    };
  ...
  const ControllerModule = (() => {
    ...
    const executeQuery = async () => {
      ...
      try {
        ...
        if ([QUERY_MODES.PLAN_CODE, QUERY_MODES.PLAN_NAME, QUERY_MODES.ALL_MASTER_PLANS, QUERY_MODES.MASTER_IN_SALE].includes(queryMode)) {
          if (queryMode === QUERY_MODES.PLAN_CODE) {
            const planCodes = utils.inputUtils.splitInput(queryInput);
            if (planCodes.length === 1) {
              const params = apiModule.buildMasterQueryParams(queryMode, planCodes[0], 1, config.DEFAULT_QUERY_PARAMS.PAGE_SIZE_MASTER);
              ...
              rawRecords = (await apiModule.queryMultiplePlanCodes(planCodes, stateModule.get().currentQueryController.signal)).records;
            }
          } else {
          ...
          const [all, current] = await Promise.all([
            apiModule.callApi(`${stateModule.getApiBase()}/planCodeController/query`, { currentPage: 1, pageSize: config.DEFAULT_QUERY_PARAMS.PAGE_SIZE_MASTER }, stateModule.get().currentQueryController.signal),
            apiModule.callApi(`${stateModule.getApiBase()}/planCodeController/query`, { saleEndDate: '9999-12-31 00:00:00', currentPage: 1, pageSize: config.DEFAULT_QUERY_PARAMS.PAGE_SIZE_MASTER }, stateModule.get().currentQueryController.signal)
          ]);
        ...
        if (error.name !== 'AbortError') {
          utils.errorHandler.handle(error, '查詢執行');
        }
      } finally {
    ...
    const updateTableDisplay = () => {
      ...
      summaryDiv.innerHTML = uiModule.Table.renderSummary(data, state.allProcessedData.some(r => r.special));
      
      // 創建新的表格元素
      const tableDiv = document.createElement('div');
      tableDiv.innerHTML = uiModule.Table.renderTableHTML(pagination.data);
      
      // 批量替換 DOM
      const summaryElement = uiRefs.body.querySelector('.pct-summary');
      const tableElement = uiRefs.body.querySelector('.pct-table-wrap');
      
      if (summaryElement) {
        summaryElement.replaceWith(summaryDiv.firstElementChild);
      }
      if (tableElement) {
        tableElement.replaceWith(tableDiv.firstElementChild);
      }
    };
    ...
    const querySinglePlanCode = async planCode => {
      try {
        ...
        utils.errorHandler.handle(error, `重新查詢 ${planCode}`);
      }
    };
